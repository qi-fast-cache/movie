    ...
    <div class="search_body">
        <div class="search_input">
            <div class="search_input_wrapper">
                ...
                <input type="text" v-model="handleInfo">
            </div>					
        </div>
        <div class="search_result">
            ...
            <ul>
                <li v-for="item in dataList" :key="item.id">
                    <div class="img"><img :src="item.img | WH(180.120)"></div>
                    <div class="info">
                        <p><span>{{item.nm}}</span><span>{{item.sr}}</span></p>
                        <p>{{item.enm}}</p>
                        <p>{{item.cat}}</p>
                        <p>{{item.frt}}</p>
                    </div>
    ...
    watch: {
        handleInfo(val){
            ...
            var timer = setTimeout(()=>{
                this.axios.get("/api/searchList?cityId=418&&kw="+val).then((res) =>{
                    var res = res.data
                    if(res.msg && res.data.movies){
